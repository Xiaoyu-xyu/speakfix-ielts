...
function schemaRepairDescription(node: "A02" | "A03") {
  ...
  if (node === "A03") {
    return {
      ...
      reasonSuggestion: "Chinese string, may be omitted and generated by server",
...
function createRawPolishFallbackResponse(
  ...
  return {
    ...
    reasonSuggestion: mockResult.reasonSuggestion,
...
export function validatePolishApiResponse(value: unknown): ApiPolishResponse | null {
  ...
  return {
    ...
    reasonSuggestion: normalizeOptionalString(value.reasonSuggestion),
...
function containsUnsafeExtension(extensionSentence: string, input: PolishInput) {
  ...
  const sensitivePatterns = [
    ...
    /\b(?:every weekend|on weekends?|last weekend)\b/,
  ];
...
function createSafeNoAnswerPolishResponse(input: PolishInput): ApiPolishResponse {
  return {
    ...
    extensionSentence: "",
    reasonSuggestion: "这次还没有真正回答题目；下次可以先直接回答问题，再补充一个简单细节。",
...
export function finalizePolishResponse({
  ...
  const result: ApiPolishResponse = {
    ...
    reasonSuggestion: normalizeOptionalString(response.reasonSuggestion),
  ...
  if (diagnosis === "low_confidence_transcript") {
    return {
      ...
      extensionSentence: "",
      reasonSuggestion:
        "转写中有一处表达不太明确，我按当前语义保守处理；下次可以先确认转写是否准确。",
      hasMeaningfulPolish: false,
    };
  }
  ...
  result.reasonSuggestion = createPolishReasonSuggestion(result, input, diagnosis);

  return result;
}

function createPolishReasonSuggestion(
  result: ApiPolishResponse,
  input: PolishInput,
  diagnosis: ReturnType<typeof diagnoseAiPolishInput>,
) {
  ...
  if (diagnosis === "meta_or_no_answer" || diagnosis === "off_topic") {
    return "这次还没有真正回答题目；下次可以先直接回答问题，再补充一个简单细节。";
  }

  const hasRed = result.originalSegments.some((segment) => segment.markType === "red");
  const hasOrange = result.originalSegments.some((segment) => segment.markType === "orange");
  const hasExtension = Boolean(result.extensionSentence.trim());
